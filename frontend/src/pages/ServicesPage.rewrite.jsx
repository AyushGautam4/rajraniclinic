...
const ServicesPage = () => {
  ...
  const categories = [
    ...
    { id: 'Featured', label: language === 'hi' ? 'मुख्य' : 'Featured' },
    { id: 'Diagnostic', label: language === 'hi' ? 'डायग्नोस्टिक' : 'Diagnostic' },
    { id: 'Specialty', label: language === 'hi' ? 'विशेषज्ञता' : 'Specialty' }
  ];
  ...
  return (
    <div className={`page-shell min-h-screen ${darkMode ? 'bg-slate-950' : 'bg-white'}`}>
      <section className={`px-4 py-4 ${darkMode ? 'bg-gradient-to-r from-teal-900/40 via-slate-900 to-blue-900/40' : 'bg-gradient-to-r from-teal-50 to-blue-50'}`}>
        <div className="container mx-auto">
          <Link
            to="/diagnostics"
            className={`flex items-center justify-between rounded-xl border p-3 transition-all hover:scale-[1.01] md:p-4 ${
          ...
          >
            <div className="flex items-center gap-3">
              <div className={`flex h-10 w-10 items-center justify-center rounded-xl md:h-12 md:w-12 ${darkMode ? 'bg-teal-500/20' : 'bg-teal-100'}`}>
                <FlaskConical className={`h-5 w-5 md:h-6 md:w-6 ${darkMode ? 'text-teal-400' : 'text-teal-600'}`} />
              </div>
              <div>
                <h3 className={`text-sm font-bold md:text-base ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                ...
                <p className={`text-[10px] md:text-xs ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                  {language === 'hi' ? 'ब्लड टेस्ट, एक्स-रे, अल्ट्रासाउंड और बहुत कुछ' : 'Blood Tests, X-Ray, Ultrasound & More'}
                </p>
              </div>
            </div>
            <ArrowRight className={`h-5 w-5 ${darkMode ? 'text-teal-400' : 'text-teal-600'}`} />
          </Link>
        </div>
      </section>
      ...
      >
        ...
        <div className="container mx-auto relative z-10 text-center">
          <Badge className={`mb-3 ${darkMode ? 'bg-teal-900/50 text-teal-300' : 'bg-teal-100 text-teal-700'}`}>{t.ourServices}</Badge>
          <h1 className={`mb-4 text-3xl font-bold md:text-5xl ${darkMode ? 'text-white' : 'text-gray-900'}`}>
            {language === 'hi' ? 'हमारी ' : 'Our '}
            <span className="bg-gradient-to-r from-teal-500 to-blue-500 bg-clip-text text-transparent">
              {language === 'hi' ? 'सेवाएं' : 'Services'}
            </span>
          </h1>
          <p className={`mx-auto mb-6 max-w-2xl text-base md:text-lg ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
            {language === 'hi'
              ? 'अनुभवी डॉक्टरों के साथ पूर्ण डायग्नोस्टिक और चिकित्सीय सुविधाएं। अब साफ, compact और premium card flow में।'
      ...
      <section className={`px-4 py-12 md:py-16 ${darkMode ? 'bg-slate-900' : 'bg-gray-50'}`}>
        <div className="container mx-auto">
                      ...
                      <div className="p-4 md:p-5">
                        ...
                        <div className="mt-4 flex items-center justify-between gap-3">
                          ...
                          {service.consultationFee > 0 ? (
                            ...
                            <div className="text-sm font-semibold text-teal-500">{language === 'hi' ? 'टेस्ट बेस्ड' : 'Test based'}</div>
                          )}
                        </div>
                        ...
                        {isTouchCardMode ? (
                          <div className={`mt-4 inline-flex rounded-full px-3 py-1 text-[11px] font-semibold ${darkMode ? 'bg-slate-900 text-teal-200' : 'bg-slate-100 text-teal-700'}`}>
                            {isActive
                              ...
                              : language === 'hi' ? 'टैप करके डिटेल देखें' : 'Tap to view details'}
                          </div>
                        ) : null}
                      </div>
                    </div>
                    ...
                    <div className={`service-slide-detail p-5 md:p-6 ${darkMode ? 'bg-slate-900/95 text-slate-100' : 'bg-white text-slate-900'}`}>
                      ...
                      <div className={`mt-5 flex items-center justify-between border-t pt-4 ${darkMode ? 'border-slate-700' : 'border-gray-200'}`}>
                        ...
                        {service.consultationFee > 0 ? (
                          ...
                          <div className="text-sm font-semibold text-teal-500">{language === 'hi' ? 'पूछताछ करें' : 'Ask for pricing'}</div>
                        )}
                      </div>
      ...
      {false && (
        ...
        >
          <div className="container mx-auto text-center">
            <h2 className="mb-4 text-2xl font-bold text-white md:text-3xl">{language === 'hi' ? 'सेवाओं की जरूरत है?' : 'Need Our Services?'}</h2>
